...
export const salesApi = {
  list: (params?: { from?: string; to?: string; paymentMethod?: string; status?: string; page?: number; limit?: number }): Promise<PaginatedResponse<Sale>> =>
    api.get('/sales', { params }),
  create: (dto: CreateSaleDto): Promise<Sale> => api.post('/sales', dto),
  getById: (id: string): Promise<Sale> => api.get(`/sales/${id}`),
  void: (id: string): Promise<Sale> => api.patch(`/sales/${id}/void`),
  statsToday: (): Promise<SalesTodayStats> => api.get('/sales/stats/today'),

...
export const dailyReportApi = {
  ...
  create: (dto: CreateDailyReportDto): Promise<DailyReport> => api.post('/daily-report', dto),
  today: (): Promise<DailyReport | null> => api.get('/daily-report/today'),
  getById: (id: string): Promise<DailyReport> => api.get(`/daily-report/${id}`),
  update: (id: string, dto: Partial<CreateDailyReportDto>): Promise<DailyReport> =>
    api.patch(`/daily-report/${id}`, dto),
  finalize: (id: string): Promise<DailyReport> => api.post(`/daily-report/${id}/finalize`),
}

...
export const expensesApi = {
  list: (params?: { expenseType?: string; from?: string; to?: string; page?: number; limit?: number }): Promise<PaginatedResponse<Expense>> =>
    api.get('/expenses', { params }),
  create: (dto: CreateExpenseDto): Promise<Expense> => api.post('/expenses', dto),
  getById: (id: string): Promise<Expense> => api.get(`/expenses/${id}`),
  update: (id: string, dto: Partial<CreateExpenseDto>): Promise<Expense> => api.patch(`/expenses/${id}`, dto),

...
export default function StockOutScreen() {
  ...
  const mutation = useMutation({
    mutationFn: (dto: { quantity: string; notes: string }) => inventoryApi.stockOut(id!, dto),
    onSuccess: () => {
  ...
  return (
    <ScreenWrapper>
      <Header title={`${t('inventory.stockOut')}: ${item.name}`} />
      ...
      <ScrollView style={{ padding: spacing[4] }}>
        ...
        >
          <Text style={styles.btnText}>{mutation.isPending ? '...' : t('inventory.stockOut')}</Text>
        </TouchableOpacity>
      </ScrollView>
    </ScreenWrapper>
  )

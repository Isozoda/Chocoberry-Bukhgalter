...
export const cashboxApi = {
  getBalance: (): Promise<CashboxBalance> =>
    api.get('/cashbox/balance'),

  getHistory: (params?: {
    page?: number
    limit?: number
  }): Promise<PaginatedResponse<CashboxOperation>> =>
    api.get('/cashbox/history', { params }),

  operation: (dto: CashboxOperationDto): Promise<CashboxOperation> =>
    api.post('/cashbox/operation', dto),
  ...
  getTodayReport: (): Promise<CashboxTodayReport> =>
    api.get('/cashbox/today'),
}

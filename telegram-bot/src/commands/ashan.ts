...
import {
  ...
  ParsedNotFoundItem,
  InventoryItemRef,
} from '../parsers/ashan-parser';
import { ashanConfirmKeyboard, ashanPickKeyboard } from '../utils/keyboards';
...
interface AshanSession {
  autoMatched: ParsedMatchedItem[];
  notFound: ParsedNotFoundItem[];
  manualMatched: ManualMatch[];
  editIndex: number;
  rawMessage: string;
}

const sessions = new Map<string, AshanSession>();
...
function fmtStock(stock: string, unit: string): string {
  const n = parseFloat(stock);
  const u = UNIT_DISPLAY[unit] || unit;
  return `${fmtNum(n)} ${u}`;
}

function getResolvedIndices(session: AshanSession): Set<number> {
  return new Set(session.manualMatched.map((mm) => mm.notFoundIndex));
}

function buildConfirmText(session: AshanSession): string {
...
export async function handleAshanMessage(
  bot: TelegramBot,
  ...
  const result = parseAshanMessage(rawMessage, items);

  if (result.matched.length === 0 && result.notFound.length === 0) {
    await bot.sendMessage(
      chatId,
      '❓ Рӯйхат шинохта нашуд. Формати дуруст:\n\n<code>Ашан\nКлубника 8,2кг\nБанан 8,4кг</code>',
      { parse_mode: 'HTML' },
    );
    return;
  }

  const session: AshanSession = {
  ...
  await bot.sendMessage(chatId, buildConfirmText(session), {
    parse_mode: 'HTML',
    reply_markup: ashanConfirmKeyboard(result.notFound.length > 0),
  });
}

export async function handleAshanConfirm(
  bot: TelegramBot,
  chatId: string,
  api: ApiService,
): Promise<void> {
  ...
  try {
    ...
    await bot.sendMessage(
      chatId,
      `❌ Хато: ${err?.response?.data?.message || err.message || 'Серверро тафтиш кунед'}`,
    );
    return;
  }
  ...
  const lowItems = (result.results ?? []).filter((r: any) => r.isLow);
...
export async function handleAshanCancel(bot: TelegramBot, chatId: string): Promise<void> {
  sessions.delete(chatId);
  await bot.sendMessage(chatId, '❌ Бекор карда шуд.');
}

export async function handleAshanEdit(bot: TelegramBot, chatId: string): Promise<void> {
  const session = sessions.get(chatId);
  if (!session) {
  ...
  if (firstUnresolved === -1) {
    await bot.sendMessage(chatId, buildConfirmText(session), {
      parse_mode: 'HTML',
      reply_markup: ashanConfirmKeyboard(false),
    });
    return;
  }
...
async function showEditStep(bot: TelegramBot, chatId: string): Promise<void> {
  const session = sessions.get(chatId);
  if (!session) return;
  ...
  while (idx < session.notFound.length && resolved.has(idx)) idx++;
  ...
  const text =
    `❓ <b>"${esc(nf.name)}"</b> — кадом мол? (${pos}/${remaining.length})\n` +
    `<i>Миқдор: ${fmtNum(nf.quantity)} ${nf.unit}</i>`;

  await bot.sendMessage(chatId, text, {
    parse_mode: 'HTML',
    reply_markup: ashanPickKeyboard(nf.suggestions),
  });
}

export async function handleAshanPick(
  bot: TelegramBot,
  chatId: string,
  itemId: string,
): Promise<void> {
...
export async function handleAshanSkip(bot: TelegramBot, chatId: string): Promise<void> {
  const session = sessions.get(chatId);
  if (!session) {
    await bot.sendMessage(chatId, '❌ Сессия ёфт нашуд.');
    return;
